...
; Original work by Aubrey Jaffer
; Copyright (c) 1992, 1993 Aubrey Jaffer
; Adapted by Teng Man
; Copyright (c) 2025 Teng man
      ...
      (let ((winds dynamic:winds))
        (oldcc
          (lambda (cont)
            (proc
              (lambda vs
                (dynamic:do-winds winds (- (length dynamic:winds) (length winds)))
                (apply cont vs)))))))))

(define (dynamic:do-winds to delta)
  (cond ((eq? dynamic:winds to) #t)
	((< delta 0)
  ...
  (set! call/cc call-with-current-continuation)
