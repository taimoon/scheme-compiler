...
(define (collect-quote e)
  (match e
    ...
    ((,form ,() . ,es)
     (guard (memq form '(foreign-call foreign-call! case-lambda)))
     (collect-quote-each es))
    ((set! ,() ,e)
     (collect-quote e))
    ((lambda ,() ,() ,() ,e)
...
(define (uncover-set! e)
  (match e
    ...
    ((,form . ,es)
     ...
     (uncover-set!-each es))
    ((,form ,() . ,es)
     (guard (memq form '(foreign-call foreign-call! case-lambda set-label!)))
     (uncover-set!-each es))
    (,()
...
(define (convert-assignment e set!-vars)
  (match e
    ...
    ((,fcall ,fn . ,es)
     ...
     `(,fcall ,fn . ,(convert-assignment-each es set!-vars)))
...
(define (convert-closure e env)
  (match e
    ...
    ((,fcall ,fn . ,es)
     (guard (memq fcall '(foreign-call foreign-call!)))
     `(,fcall ,fn . ,(convert-closure-each es env)))
...
(define (expand-operand-prim e)
  (match e
    ...
    ((,fcall ,fn . ,es)
     (guard (memq fcall '(foreign-call foreign-call!)))
     `(,fcall ,fn . ,(map expand-operand-prim es)))
    ((let ,bs ,e)
  ...
  (match prog
    ((program (labels . ,codes) (data . ,data) (unbounds . ,unbounds) ,e)
     `(program
        ...
        (unbounds . ,unbounds)
        ,(anf-of e identity)))
    (,() (error "anf-of-program" "unmatch" prog))))
...
(define (expose-alloc e)
  (match e
    ...
    ((closure-ref ,clos ,i)
     e)
    ((foreign-call! ,fn . ,es)
     `(begin (collect) (foreign-call! ,fn . ,es)))
    ((foreign-call ,fn . ,es)
