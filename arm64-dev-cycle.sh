...
rsync -avz -e "ssh -p 22225" --exclude "*.out" --exclude "qemu-env" --exclude ".git" . root@localhost:~/scheme-compiler &&\
ssh -p 22225 root@localhost "\
    cd ~/scheme-compiler/ &&\
    source activate.sh &&\
...
rsync -avz -e "ssh -p 22225" root@localhost:~/scheme-compiler/compiler-arm64.out . &&\
exit 0
