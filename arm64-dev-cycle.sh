...
rsync -avz -e "ssh -p 22225" --exclude "*.out" --exclude "qemu-env" --exclude ".git" $PWD root@localhost:~
...
rsync -avz -e "ssh -p 22225" root@localhost:~/scheme-compiler-v2/compiler-arm64.out .
exit 0
