...
rsync -avz -e "ssh -p 22224" --exclude "*.out" --exclude "qemu-env" --exclude ".git" . root@localhost:~/scheme-compiler &&\
...
rsync -avz -e "ssh -p 22224" root@localhost:~/scheme-compiler/compiler-riscv64.out . &&\
exit 0